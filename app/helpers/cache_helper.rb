module CacheHelper
  ...
  def cache_key_for_current_auction_stats(auction)
    id = auction.id
    items_last_updated = auction.auction_items.maximum(:updated_at).try(:utc).try(:to_s, :number)
    "auctions/current/stats-#{id}-#{items_last_updated}"
  end
